...
<div id="sidebar" class="fixed inset-y-0 left-0 z-30 w-64 bg-white shadow-lg transform -translate-x-full transition-all duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 flex flex-col border-r border-gray-200">
    ...
    <div class="flex items-center justify-between h-16 px-4 border-b border-gray-200">
        <div class="flex items-center space-x-3">
            ...
            <span class="text-lg font-semibold text-gray-800 sidebar-text">Panel Admin Kursus</span>
        </div>
    ...
    <nav class="flex-1 px-4 py-4 overflow-y-auto">
        <ul class="space-y-2">
            <!-- Menu Panel Admin Kursus -->
            <li>
                <a href="{{ route('admin.dashboard') }}" class="flex items-center px-3 py-2.5 text-sm font-medium text-gray-700 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 group">
                    <i class="fas fa-home w-5 h-5 mr-3 text-gray-500 group-hover:text-blue-500"></i>
                    <span class="sidebar-text">Dashboard</span>
                </a>
            </li>
            <li class="relative">
                <button onclick="toggleDropdown('management')" class="flex items-center justify-between w-full px-3 py-2.5 text-sm font-medium text-gray-700 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 group">
                    <div class="flex items-center">
                        <i class="fas fa-users-cog w-5 h-5 mr-3 text-gray-500 group-hover:text-blue-500"></i>
                        <span class="sidebar-text">Management</span>
                    </div>
                    <i class="fas fa-chevron-down text-xs text-gray-400 group-hover:text-blue-500 transition-all duration-200 sidebar-arrow" id="dropdown-arrow-management"></i>
                </button>
                <!-- Dropdown Content -->
                <div id="dropdown-content-management" class="hidden mt-1 ml-8 space-y-1 dropdown-content">
            ...
            <!-- Dropdown Course -->
            <li class="relative">
                <button onclick="toggleDropdown('course')" class="flex items-center justify-between w-full px-3 py-2.5 text-sm font-medium text-gray-700 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 group">
                    <div class="flex items-center">
                        <i class="fas fa-chalkboard-teacher w-5 h-5 mr-3 text-gray-500 group-hover:text-blue-500"></i>
                        <span class="sidebar-text">Management Course</span>
                    </div>
                    <i class="fas fa-chevron-down text-xs text-gray-400 group-hover:text-blue-500 transition-all duration-200 sidebar-arrow" id="dropdown-arrow-course"></i>
                </button>
                <!-- Dropdown Content -->
                <div id="dropdown-content-course" class="hidden mt-1 ml-8 space-y-1 dropdown-content">
                    <a href="{{ route('admin.kategoris.index') }}" class="flex items-center px-3 py-2 text-sm text-gray-600 rounded-md hover:bg-gray-100 hover:text-blue-600 transition-all duration-200">
                        <i class="fas fa-tags w-4 h-4 mr-3"></i>
                        <span class="sidebar-text">Kategori</span>
                    </a>
                    <a href="{{ route('admin.kursus.index') }}" class="flex items-center px-3 py-2 text-sm text-gray-600 rounded-md hover:bg-gray-100 hover:text-blue-600 transition-all duration-200">
                        <i class="fas fa-book w-4 h-4 mr-3"></i>
                        <span class="sidebar-text">Kursus Reguler</span>
                    </a>
                </div>
            </li>
           <li>
                <a href="{{ route('admin.transaksi.index') }}" class="flex items-center px-3 py-2.5 text-sm font-medium text-gray-700 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 group">
                    <i class="fas fa-receipt w-5 h-5 mr-3 text-gray-500 group-hover:text-blue-500"></i>
...
function toggleDropdown(type) {
    ...
    let dropdown, arrow;
    if (type === 'management') {
        dropdown = document.getElementById('dropdown-content-management');
        arrow = document.getElementById('dropdown-arrow-management');
    } else if (type === 'course') {
        dropdown = document.getElementById('dropdown-content-course');
        arrow = document.getElementById('dropdown-arrow-course');
    }
    if (dropdown && arrow) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(event) {
        const dropdownManagement = document.getElementById('dropdown-content-management');
        const arrowManagement = document.getElementById('dropdown-arrow-management');
        const dropdownCourse = document.getElementById('dropdown-content-course');
        const arrowCourse = document.getElementById('dropdown-arrow-course');
        const userDropdown = document.getElementById('user-dropdown-content');

        // Close Management dropdown if click outside
        if (!event.target.closest('.relative') && dropdownManagement && !dropdownManagement.classList.contains('hidden')) {
            dropdownManagement.classList.add('hidden');
            arrowManagement?.classList.remove('rotate-180');
        }
        // Close Course dropdown if click outside
        if (!event.target.closest('.relative') && dropdownCourse && !dropdownCourse.classList.contains('hidden')) {
            dropdownCourse.classList.add('hidden');
            arrowCourse?.classList.remove('rotate-180');
        }
