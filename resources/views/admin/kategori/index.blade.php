...
<div x-data="{ 
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ auth()->user()->role === 'admin' ? 'Manajemen Kategori' : 'Daftar Kategori' }}</h1>
                <p class="text-sm text-gray-500 mt-1">{{ auth()->user()->role === 'admin' ? 'Kelola kategori sistem' : 'Informasi kategori sistem' }}</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                {{-- Search Form --}}
                <form method="GET" action="{{ auth()->user()->role === 'admin' ? route('admin.kategoris.index') : route('user.kategori.index') }}" class="flex gap-2">
                    <div class="relative">
                ...
                @forelse($kategoris as $kategori)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                            ...
                            {{-- Delete Button --}}
                            <form action="{{ route('admin.kategoris.destroy', $kategori) }}" method="POST" class="inline delete-form">
                                @csrf @method('DELETE')
                        ...
                        <div @click.away="toggleModal('detail', {{ $kategori->id }})" 
                            ...
                            <div class="px-6 py-6">
                                <div class="text-center mb-6">
                                    ...
                                    <p class="text-sm text-gray-500">{{ $kategori->kode }}</p>
                                    <p class="text-sm text-gray-500">{{ $kategori->deskripsi }}</p>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        <dt class="text-sm font-medium text-gray-500">Kategori ID</dt>
                                        <dd class="mt-1 text-sm font-mono text-gray-900">#{{ str_pad($kategori->id, 4, '0', STR_PAD_LEFT) }}</dd>
                                    </div>
                                    <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div x-show="editModals[{{ $kategori->id }}]" x-cloak 
                        ...
                        <div @click.away="toggleModal('edit', {{ $kategori->id }})" 
                            ...
                            <div class="px-6 py-4">
                                <form method="POST" action="{{ route('admin.kategoris.update', $kategori) }}" class="space-y-4">
                                    @csrf @method('PUT')
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Kode</label>
                                            <input type="text" name="kode" value="{{ $kategori->kode }}" class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Nama</label>
                                            <input type="text" name="nama" value="{{ $kategori->nama }}" class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                                            <textarea name="deskripsi" class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>{{ $kategori->deskripsi }}</textarea>
                                        </div>
                                    </div>
                                    <div class="flex justify-end space-x-3 pt-4">
                                        <button type="button" @click="toggleModal('edit', {{ $kategori->id }})" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors font-medium">Batal</button>
    ...
    <div x-show="openCreate" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div @click.away="toggleModal('create')" class="bg-white rounded-xl shadow-xl w-full max-w-lg">
            ...
            <div class="px-6 py-4">
                <form method="POST" action="{{ route('admin.kategoris.store') }}" id="createForm" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">kode</label>
                            <input type="text" name="kode" value="{{ old('kode') }}" class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Jangan Di isi !" readonly required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Nama</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                            <textarea name="deskripsi" class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Masukkan deskripsi" required>{{ old('deskripsi') }}</textarea>
                        </div>
                    </div>
                </form>
            </div>
